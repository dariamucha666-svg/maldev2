...
2. `apkid` — wykrycie packera/kompilatora DEX.
...
7. Generacja **YARA** (UTF-16 package — bo resources.arsc jest Stored) + **karta Obsidian** (.md).
...
`/tmp/apk_analysis/<sha256-pierwsze12>/` → karta `.md`, reguła `.yar`, `jadx_out/`, `manifest.xml`, `file.txt`.
